...
public class TokenEndpoint extends AbstractEndpoint {
	...
	Map<String, String> parameters) throws HttpRequestMethodNotSupportedException {
		...
		logger.info("[TokenEndpoint] POST /oauth/token invoked");
		logger.info("[TokenEndpoint] parameters = {}", parameters);

		String clientId = parameters.containsKey("client_id") ? parameters.get("client_id") : "anonymous";
		boolean privacyMode = "anonymous".equals(clientId);
		logger.info("[TokenEndpoint] client_id = {}, privacyMode = {}", clientId, privacyMode);
		...
		logger.info("[TokenEndpoint] token generated = {}", token);

		if (token == null) {
			throw new OAuth2Exception("Unsupported grant type: " + tokenRequest.getGrantType());
		}

		logger.info("[TokenEndpoint] responding with token");
