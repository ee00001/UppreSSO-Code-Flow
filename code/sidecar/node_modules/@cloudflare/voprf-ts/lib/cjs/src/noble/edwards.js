"use strict";
// Copyright (c) 2021 Cloudflare, Inc. and contributors.
// Copyright (c) 2021 Cloudflare, Inc.
// Licensed under the BSD-3-Clause license found in the LICENSE file or
// at https://opensource.org/licenses/BSD-3-Clause
Object.defineProperty(exports, "__esModule", { value: true });
exports.edwardsCurve = edwardsCurve;
const modular_1 = require("@noble/curves/abstract/modular");
function edwardsCurve({ curve, hashID, scalarHash, element, hash }) {
    // Leave bitLen undefined (and not curve.CURVE.nBitLength) as ed448 sets it to
    // 448+8 = 456 internally so scalars will be output in the "wrong" size
    const scalarField = (0, modular_1.Field)(curve.CURVE.n, undefined, true);
    // Ditto
    const elementField = (0, modular_1.Field)(curve.CURVE.p);
    return {
        isEdwards: true,
        scalar: {
            field: scalarField,
            size: scalarField.BYTES,
            hash: scalarHash
        },
        element: {
            ...element,
            size: {
                compressed: elementField.BYTES,
                standard: elementField.BYTES
            }
        },
        hash: {
            id: hashID,
            size: hash.outputLen,
            fn: hash
        }
    };
}
//# sourceMappingURL=edwards.js.map